...
namespace OCMaestro
{
    public class Ops : IDisposable
    {
        ...
        public void CheckOlsStatus()
        {
            ...
            {
                OCMaestro.OlsInit(0);
                OCMaestro.MsgLog("WinRing initalization failed: " + e.GetType().Name, EventId.init, EventLogEntryType.Warning);
            }
    }
        ...
        /*
        public bool MSmuWrite(uint addr, uint data)
        {
            bool res = false;
            amdSmuMutex.WaitOne(5000);
            if (MSmuWriteRegNM(SMUMatisse.SMU_ADDR_RSP, 0U))
            {
                if (this.MSmuWriteRegNM(SMUMatisse.SMU_ADDR_ARG, data))
                {
                    this.MSmuWriteRegNM(SMUMatisse.SMU_ADDR_ARG + (uint)(i * 4), 0U);
                }
                if (this.MSmuWriteRegNM(SMUMatisse.SMU_ADDR_MSG, addr))
                {
                    // Wait done
                    if (!SmuWaitDone())
                    {
                    ...
                    res = this.SMU_Wait_Response();
                }
            }
            if (Ols.WritePciConfigDwordEx(Smu.SMU_PCI_ADDR, Smu.SMU_OFFSET_ADDR, addr) == 1)
                res = (Ols.WritePciConfigDwordEx(Smu.SMU_PCI_ADDR, Smu.SMU_OFFSET_DATA, data) == 1);
            amdSmuMutex.ReleaseMutex();
            OCMaestro.MsgLog("smu_write=" + res.ToString() + " addr=" + addr.ToString() + " data=" + data.ToString());
            return res;
        }
        public bool MSmuWriteRegNM (uint addr, uint data)
        {
            bool res = false;
            if (Ols.WritePciConfigDwordEx(Smu.SMU_PCI_ADDR, Smu.SMU_OFFSET_ADDR, addr) == 1)
                res = (Ols.WritePciConfigDwordEx(Smu.SMU_PCI_ADDR, Smu.SMU_OFFSET_DATA, data) == 1);
        ...
        */
        ...
        public int GetCCDCount()
        {
            ...
            s1 = string.Format(s1, value1.ToString("X"));
            s2 = string.Format(s2, value2.ToString("X"));
            s3 = string.Format(s3, value3.ToString("X"));

            OCMaestro.MsgLog("SMU1 1=" + s1.ToString() + " 2=" + s2.ToString() + " 3=" + s3.ToString());
            
            OCMaestro.MsgLog("SMU2 1=" + value1.ToString() + " 2=" + value2.ToString() + " 3=" + value3.ToString());

            value1 = (value1 >> 22) & 0xff;
            value2 = (value2 >> 30) & 0xff;
            value3 &= 0x3f;

            OCMaestro.MsgLog("SMU3 1=" + value1.ToString() + " 2=" + value2.ToString() + " 3=" + value3.ToString());
            ...
            s2 = string.Format(s2, value2.ToString("X"));
            s3 = string.Format(s3, value3.ToString("X"));
            s4 = string.Format(s4, value4.ToString("X"));

            OCMaestro.MsgLog("SMU4 1=" + s1.ToString() + " 2=" + s2.ToString() + " 3=" + s3.ToString() + " 4=" + s4.ToString());
